...
# INPUT FILES
func <- as.data.frame(t(readxl::read_xlsx("tax4fun_functional.xlsx")))  #tax4fun pathway abundance output
genus_t <- read.table('genus_t.tsv')

# Clean the input
rownames(func) <- gsub('^X', '', rownames(func))
colnames(func) <- func['description',]
path <- as.data.frame(t(readxl::read_xlsx("tax4fun_pathway.xlsx")))
rownames(path) <- gsub('^X', '', rownames(path))
...
# corr_tab <- corr_tab[,-which(colnames(corr_tab) %in% c("uncharacterized.protein.52",
#                                                        "protein.sgcE..EC.5.1.3...",))]
...
colnames(corr_tab)[colnames(corr_tab) == "shikimate.dehydrogenase..EC.1.1.1.25."] <- "chorismate biosynthesis"
colnames(corr_tab)[colnames(corr_tab) == "non.specific.riboncleoside.hydrolase..EC.3.2....."] <- "ribonucleoside hydrolysis"
colnames(corr_tab)[colnames(corr_tab) == "flavin.reductase..NADH..subunit.1..EC.1.5.1.36."] <- "flavin:NADH reduction"
colnames(corr_tab)[colnames(corr_tab) == "two.component.system..OmpR.family..response.regulator.BasR"] <- "signal transduction"
colnames(corr_tab)[colnames(corr_tab) == "maltose.phosphorylase..EC.2.4.1.8."] <- "maltose degradation"
colnames(corr_tab)[colnames(corr_tab) == "X23S.rRNA..adenine2503.C2..methyltransferase..EC.2.1.1.192."] <- "methyl groups tansfer"
colnames(corr_tab)[colnames(corr_tab) == "nucleoside.deoxyribosyltransferase..EC.2.4.2.6."] <- "nucleoside deoxyribosyltransferase"
colnames(corr_tab)[colnames(corr_tab) == "probable.pyridine.nucleotide.disulfide.oxidoreductase"] <- "thiol residues reduction"
...
top_corr <- function(corr_sub, n = 8){
  ...
  # Sort and select the top 10 unique columns
...
png("func_t4f.png", units="in", width=8, height=5, res=300)
