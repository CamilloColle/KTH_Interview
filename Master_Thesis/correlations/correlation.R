...
metadata_clean <- metadata[,c("Subject", "SCFA.", "BA.", "Water.content....",
...
# Uncomment to save this table
#write.table(genus.int_t, file = 'genus_t.tsv', sep = '\t')
...
corr_sub <- cor_matrix[c(1:6),-c(1:6)]
...
df.groups <- c(rep("Stool", 3), rep("Diet", 3), rep("Taxa", 8))
...
# with 3 entries, a "Diet" group with 3 entries and a "Taxa" group with 8 entries, 
# we will need to create a colors vector which has the color for each of the groups 
# repeated for the same number of times, i.e. 3, 3 and 8 in this case
sec_cols <- c(rep("#C55A11", 3), rep("#548235", 3), rep("#660066", 8))
...
png("chord_top_corr.png", units="in", width=8, height=5, res=300)
...
lgd_diet = Legend(title = "Diet", 
                  labels =  names(df.groups[df.groups == "Diet"]),
                  type = "points", 
                  pch = as.character(1:3), #change this range according to the number of entries in each sector (in this case 3)
                  legend_gp = gpar(col = "white", cex = 0.7),
                  background = group_cols[names(group_cols) == "Diet"])

lgd_stool = Legend(title = "Stool", 
                  labels =  names(df.groups[df.groups == "Stool"]),
                  type = "points", 
                  pch = as.character(4:6), #change this range according to the number of entries in each sector (in this case 3)
                  legend_gp = gpar(col = "white", cex = 0.7),
...
lgd_taxa = Legend(title = "Taxa", 
                  ...
                  type = "points", 
                  pch = as.character(7:14), #change this range according to the number of entries in each sector (in this case 8)
                  legend_gp = gpar(col = "white", cex = 0.7),
...
lgd_list_vertical = packLegend(lgd_diet, lgd_stool, lgd_taxa, lgd_links)
lgd_list_vertical
