...
metadata_clean <- metadata[,c("Subject", "Crude.protein", 
                              "Crude.fibre", "Crude.fat", "Carbohydrates",
...
corr_sub <- cor_matrix[c(1:9),-c(1:9)]
...
df.groups <- c(rep("Diet", 9), rep("Taxa", 8))
...
# Define colors for the groups. Note that this must be done manually, meaning if we have a "Diet" group
# with 9 entries and a "Taxa" group with 8 entries, we will need to create a colors vector which has the color
# for each of the groups repeated for the same number of times, i.e. 9 and 8 in this case
sec_cols <- c(rep("#548235", 9), rep("#660066", 8))
names(sec_cols) <- sectors

# Associate group names with colors
group_cols <- c(Diet = "#548235", Taxa = "#660066")
...
png("chord_diet.png", units="in", width=8, height=5, res=300)
...
lgd_diet = Legend(title = "Diet", 
                  ...
                  type = "points", 
                  pch = as.character(1:9), #change this range according to the number of entries in each sector (in this case 9)
...
lgd_taxa = Legend(title = "Taxa", 
                  ...
                  type = "points", 
                  pch = as.character(10:17), #change this range according to the number of entries in each sector (in this case 8)
                  legend_gp = gpar(col = "white", cex = 0.7),
...
lgd_list_vertical = packLegend(lgd_diet, lgd_taxa, lgd_links)
lgd_list_vertical
